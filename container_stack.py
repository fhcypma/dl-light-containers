...
class ContainerStack(BasicStack):
    def __init__(
        self, scope: Construct, build_number: str, dtap: str, etl_account: str, **kwargs
    ) -> None:
        super().__init__(scope, "ContainerStack", dtap, **kwargs)

        # Create repo
        repo = ecr.Repository(
            self,
            "SparkOnLambdaRepo",
            repository_name=f"{dtap}-spark-on-lambda/base",
